...
async function sincronizarNuvemParaLocal(registrosLocais) {
    try {
        ...
        if (registrosNuvem.length === 0) return registrosLocais;
        ...
        // Mesclar: pegar todos os IDs únicos
        const todosIds = new Set([...Object.keys(mapaLocal), ...Object.keys(mapaNuvem)]);
        const resultado = [];
        ...
        for (const id of todosIds) {
            ...
            if (local && nuvem) {
                ...
                if (dataNuvem > dataLocal) {
                    // Nuvem mais recente: usar dados da nuvem mas manter arquivo/pasta locais
                    resultado.push({
                        ...nuvem,
